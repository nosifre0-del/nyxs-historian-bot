...
class Memory:
    ...
    @staticmethod
    def _cmd(*args):
        """ينفذ أمر Redis عبر Upstash REST API بصيغة command-style الرسمية.
        مثال: Memory._cmd("SET", "key", "value", "EX", "604800")
        """
        try:
            r = requests.post(
                UPSTASH_URL,
            ...
            if not r.ok:
                print("MEMORY CMD ERROR:", r.status_code, r.text[:200])
                return None
            return r.json().get("result")
        except Exception as e:
            print("MEMORY CMD EXCEPTION:", e)
            return None

    @staticmethod
    def _get_raw(key):
        return Memory._cmd("GET", key)

    @staticmethod
    def _set_raw(key, value, ex=None):
        if ex:
            Memory._cmd("SET", key, value, "EX", str(ex))
        else:
            Memory._cmd("SET", key, value)
    ...
    def clear_history(chat_id):
        if not Memory._enabled():
            return
        Memory._cmd("DEL", f"history:{chat_id}")
