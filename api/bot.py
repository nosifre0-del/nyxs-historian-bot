...
class ImageEngine:
    ...
    STYLES = {
        "واقعي": "photorealistic, ultra realistic, professional photography, realistic textures, natural lighting",
        "سينمائي": "cinematic photography, cinematic lighting, dramatic composition, movie still, volumetric lighting, film grain",
        "انمي": "high quality anime artwork, detailed anime illustration, polished linework, beautiful colors",
        "رسم": "professional digital painting, concept art, detailed illustration, painterly texture",
        "زيتي": "traditional oil painting, thick visible brush strokes, rich pigments, textured canvas, expressive painterly surface",
        "خيال": "epic fantasy concept art, magical atmosphere, grand environment, intricate details",
        "فانتازيا": "fantasy artwork, magical world, epic environment, atmospheric lighting",
        "رعب": "dark horror atmosphere, eerie lighting, psychological horror, unsettling environment",
        "مستقبلي": "futuristic sci-fi environment, advanced technology, futuristic architecture, neon atmosphere",
        "معماري": "professional architectural visualization, precise geometry, realistic materials, architectural photography",
        "بورتريه": "professional portrait photography, realistic skin texture, detailed face, studio lighting, shallow depth of field",
        "وثائقي": "documentary photography, authentic atmosphere, natural lighting, realistic environment",
    }

    QUALITY = {
        "سريع": "clean composition, detailed",
        "عالي": "highly detailed, professional quality, refined composition, detailed textures",
        "فائق": "ultra detailed, extremely detailed, professional masterpiece, sophisticated lighting, refined textures",
    }
    ...
    def analyze_prompt(prompt):
        ...
        style_keywords = {
            ...
            "مستقبلي": ["مستقبلي", "future", "futuristic", "سايبربانك", "cyberpunk"],
            "بورتريه": ["بورتريه", "portrait", "وجه", "صورة شخصية"],
            "معماري": ["معماري", "architecture", "مبنى", "مباني"],
            "وثائقي": ["وثائقي", "documentary"],
            "واقعي": ["واقعي", "حقيقي", "realistic", "photorealistic"],
        }
        ...
        if any(x in text for x in ["يوتيوب", "youtube", "فيديو", "شاشة"]):
            ratio = "16:9"
        elif any(x in text for x in ["ستوري", "story", "ريلز", "reels", "تيك توك", "tiktok"]):
            ratio = "9:16"
        elif any(x in text for x in ["بورتريه", "portrait", "عمودي"]):
            ratio = "3:4"
        else:
            ratio = "1:1"
        ...
        if any(x in text for x in ["ليل", "ليلاً", "ليلًا", "night"]):
            lighting.append("night lighting")

        if any(x in text for x in ["غروب", "sunset"]):
            lighting.append("golden sunset lighting")

        if any(x in text for x in ["شروق", "sunrise"]):
            lighting.append("soft sunrise lighting")

        if any(x in text for x in ["مطر", "rain"]):
    ...
    @staticmethod
    def build_prompt(prompt, style=None, quality="فائق", negative=None):

        analysis = ImageEngine.analyze_prompt(prompt)

        detected_style = style or analysis["style"] or "واقعي"

        parts = [prompt]

        if detected_style in ImageEngine.STYLES:
            parts.append(ImageEngine.STYLES[detected_style])

        if quality in ImageEngine.QUALITY:
            parts.append(ImageEngine.QUALITY[quality])
    ...
    def parse_command(text):
        ...
        while i < len(args):
            ...
            if token == "--count":
                if i + 1 < len(args):
                    try:
                        count = max(1, min(int(args[i + 1]), 4))
                    except Exception:
                        count = 1
                i += 2
                continue
...
إذا طلب المستخدم الاختصار → اختصر.
إذا طلب التفصيل → توسع.
إذا طلب الرسمية → كن رسميًا.
إذا طلب العفوية → كن عفويًا.
إذا طلب الفلسفة → استخدم أسلوبًا فلسفيًا.
إذا طلب الأدب → استخدم أسلوبًا أدبيًا.
...
في الأمن السيبراني:
يمكنك المساعدة في أمن المعلومات والشبكات وLinux وCTF
والمختبرات والتحليل الدفاعي واختبار الأنظمة المصرح بها.

لا تنشئ أدوات لاختراق الحسابات الحقيقية أو سرقة كلمات المرور
أو Cookies أو Tokens أو Sessions أو Malware أو Ransomware
...
"""
    ...
    def ask(messages):
        ...
        if not AI_API_KEY.strip():
            return (
                "❌ لم يتم وضع AI API Key.\n"
                "ضع المفتاح داخل AI_API_KEY في أول سطر."
            )

        url = f"{AI_BASE_URL}/responses"
        ...
        # تحويل سجل المحادثة إلى input متوافق مع Responses API
        input_messages = [
            {
                "role": "developer",
        ...
        try:
            ...
            # الطريقة القياسية في Responses API
            output_text = data.get("output_text")

            if output_text:
                return output_text.strip()

            # احتياط في حال لم يرجع output_text مباشرة
            ...
            print(
                f"OPENAI RESPONSE ERROR: {e}"
            )

            return (
                "❌ حدث خطأ أثناء معالجة استجابة الذكاء الاصطناعي."
            )
...
def handle_image_command(chat_id, text):
    ...
    for index in range(count):
        ...
        caption = (
            "🎨 NYXS Image Engine\n\n"
            f"الوصف: {prompt}\n"
            f"النمط: {style or 'تلقائي'}\n"
            f"النسبة: {ratio or 'تلقائي'}\n"
            f"الجودة: {quality}\n"
            f"الحجم: {width}×{height}\n"
            f"Seed: {current_seed}"
        )

        if count > 1:
            caption += (
                f"\nالنسخة: {index + 1}/{count}"
            )
...
class BotCore:
    ...
    def handle_text(chat_id, text):
        ...
        if cleaned.lower() in ("/clear", "/reset"):
