...
# تم ضبط الموديل الافتراضي إلى البديل الأسرع والأكثر استقراراً gpt-4o-mini
...
class AIEngine:
    ...
    def ask(messages):
        ...
        try:
            ...
            if not response.ok:
                return f"❌ OpenAI Error ({response.status_code}): {response.text[:300]}"
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"NYXS Intelligence Core Online (gpt-4o-mini)")
    ...
    def do_POST(self):
        try:
            ...
            if text.startswith("/start"):
                send_message(chat_id, "🤖 NYXS Intelligence (gpt-4o-mini) متصل ويعمل بكفاءة مطلقة.")
                return
